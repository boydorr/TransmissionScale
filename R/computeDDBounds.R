#' @title Computes confidence intervals around distribution of distinct dogs
#' @description A horribly unwieldy piece of code, but is only run once! Uses bootstrapping approach
#' @export
computeDDBounds <- function(nBites, ci = 0.95, hist.breaks, n.draws = 10000, n.to.draw, sd.jitter=0.01) {
  
  h.data <- hist(as.numeric(nBites), breaks=hist.breaks, plot=F)
  hist.mids <- h.data$mids
  
  # Draw, with replacement, the n.to.draw case densities
  my.draws <- matrix(nrow = n.draws, ncol = n.to.draw)
  for (rr in 1:n.draws) {
    sampled.data <- sample(nBites, size = n.to.draw, replace = T)
  ...
  plot(my.bounds$cases, my.bounds$ub, type="l", lty=3, xlab="Number bitten", ylab="Relative frequency")
