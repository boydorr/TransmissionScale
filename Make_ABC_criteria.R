...
# Process_ABC.R
ff_output = "/Volumes/SimOutput01/Rabies_Sim_Output/abc_criteria/"
ff_input = "/Volumes/SimOutput01/Rabies_Sim_Output/"
...
d.cases.java.reallocated <- read.csv("output/d_cases_java.csv")
# Step 1: done in Java
# Step 2
rabid.dogs <- read.csv(paste0("/Volumes/SimOutput01/Rabies_Sim_Output/Round0_1000/postprocessing/densitiesData.csv"))
...
# Load up information on cases - bounds on the distribution of distinct dogs bitten (distinct dogs rather than distinct animals bitten because we have no reason to believe that distinct animals
#      relate to dog density; these are therefore under-estimates of the number of epidemiologically-relevant contacts, but the bounds should help account for this.)
source("R/computeDDPointsOutside.R")
case_list_rda <- readRDS(file = "output/matching_data_caselist.rda")
dim(case_list_rda)
...
# We need to either bootstrap from this, or fit a distribution and then compute the bounds from that ... but we have the data, so let's bootstrap
...
# 0.00214669 * 2795 = 5.99, so we can have a maximum of about 5-6 cases at each value where none exist in the data
